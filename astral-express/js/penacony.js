...
function initPenaconyScene() {
    ...
    function generateMathProblem() {
        ...
        return { equation: `${num1} ${operator} ${num2} = ?`, answer };
    }
    ...
    function showCoinsPhase(options, correctAnswer) {
        ...
        options.forEach(value => {
            ...
            coin.addEventListener('click', () => {
                ...
                document.querySelectorAll('.coin').forEach(c => c.classList.add('coin-hide'));
                setTimeout(() => insertCoinPhase(value, correctAnswer), 500);
            });
    ...
    function insertCoinPhase(selectedValue, correctAnswer) {
        ...
        setTimeout(() => {
            if (selectedValue === correctAnswer) {
                ...
                setTimeout(() => {
                    if (gameState.problemsSolved >= totalProblems) completeGame();
                    else resetGame();
                }, 2000);
            } else {
                penaconyFeedback.textContent = `Wrong! The answer was ${correctAnswer}. (${gameState.problemsSolved}/${totalProblems} shards)`;
        ...
        slotStartBtn.addEventListener('click', () => {
            if (gamePhase === 'completed') {
                ...
                for (let i = 1; i <= totalProblems; i++) {
                    ...
                    if (shard) { shard.classList.remove('collected'); shard.classList.add('locked'); }
                }
                gamePhase = 'start';
            ...
            setTimeout(() => {
                ...
                setTimeout(() => showCoinsPhase(generateCoinOptions(currentAnswer), currentAnswer), 500);
            }, 800);
        });
    }
}
